...
const UploadProduct = ({
    ...
    const handleSubmit = async (e) => {
        ...
        // console.log("Result of product upload >> ", result);
        ...
        <div className=' fixed top-0 bottom-0 left-0 right-0 flex justify-center items-center bg-slate-200 bg-opacity-50'>
            <div className=' bg-white w-full max-w-2xl h-full max-h-[80%] rounded p-4 overflow-hidden'>
                ...
                <form className=' grid gap-2 p-4 overflow-y-scroll h-full pb-10' onSubmit={handleSubmit}>
                    ...
                    <label htmlFor='uploadImageInput'>
                        <div className=' h-32 bg-slate-200 p-2 border rounded w-full flex items-center justify-center cursor-pointer '>
                            <div className=' text-slate-500 flex items-center justify-center flex-col gap-2'>
                                ...
                                <input type='file' id='uploadImageInput' className=' hidden' onChange={handleUploadProductImage}></input>
                            </div>
                        </div>
                    </label>
                                    ...
                                    return (
                                        <div className=' relative p-2 group'>
                                            <div className=' h-[100px] w-[100px] '>
                                                <img src={elem} alt='productImage' key={index} className=' border bg-slate-200 cursor-pointer rounded h-full w-full object-contain'
                                                    onClick={() => {
                    ...
                    <button type='submit' className=' bg-red-600 p-2 text-white hover:bg-red-700 mb-3 rounded'>Upload Product</button>
                </form>
            </div>
            ...
            {/* Display image in full screen  */}
            {
                openFullScreenDisplayBox && (
                    <FullScreenDisplay
                        ...
                        onClose={() => setOpenFullScreenDisplayBox(false)}>
                    </FullScreenDisplay>
                )
            }
        </div>
