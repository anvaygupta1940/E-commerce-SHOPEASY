            ...
            <div className=' flex items-center gap-5 overflow-scroll scrollbar-none transition-all' ref={scrollElem}>
                ...
                {loading ? (
                    loadingList.map((el, index) => {
                        return (
                            <div className=' w-full min-w-[280px] max-w-[280px] md:max-w-[320px] md:min-w-[320px] h-36 shadow-sm flex rounded-md bg-white' key={index}>
                                <div className=' bg-slate-200 min-w-[120px] md:min-w-[145px] h-full p-2 animate-pulse'>
                                </div>
                                <div className=' p-2 w-full grid gap-2 '>
                                    <p className=' text-ellipsis line-clamp-1 text-lg font-semibold p-2 rounded-full animate-pulse bg-slate-200'></p>
                                    <p className=' capitalize text-slate-500 font-medium p-2 rounded-full animate-pulse bg-slate-200'></p>
                                    <div className=' flex items-center gap-3 w-full'>
                                        <p className=' text-red-500 bg-slate-200 p-2 rounded-full animate-pulse w-full'></p>
                                        <p className=' text-slate-500 line-through  bg-slate-200 p-2 rounded-full animate-pulse w-full'></p>
                                    </div>
                                    <button className=' px-3 py-1  w-full rounded-full text-white bg-slate-200  animate-pulse p-2'></button>
                                </div>
                            </div>
                        )
                    })
                    ...
                    data.map((product, index) => {
                        return (
                            <Link to={"product/" + product?._id} className=' w-full min-w-[280px] max-w-[280px] md:max-w-[320px] md:min-w-[320px] h-36 shadow-sm flex rounded-md bg-white' key={index}>
                                <div className=' bg-slate-200 min-w-[120px] md:min-w-[145px] h-full p-2'>
                                    <img alt='productImage' src={product?.productImage[0]} className='object-scale-down h-full hover:scale-110 transition-all'></img>
                                </div>
                                <div className=' p-2 w-full flex flex-col gap-2 '>
                                    ...
                                    <button
                                        ...
                                        className=' px-3 py-1 bg-red-500 hover:bg-red-700 w-full rounded-full text-white'>Add to Cart</button>
                                </div>
                            </Link>
                        )
                    })
